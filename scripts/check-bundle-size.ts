...
interface PackageThreshold {
  /** Package name or directory. */
  name: string;
  /** Max size in bytes for the main bundle (.js). */
  maxMain: number;
  /** Max size in bytes for the full build (all .js files). */
  maxTotal: number;
}
...
function analyzePackage(threshold: PackageThreshold): BundleInfo | null {
...
function checkBundle(info: BundleInfo, threshold: PackageThreshold): { pass: boolean; warnings: string[] } {
  const warnings: string[] = [];
  let pass = true;

  if (info.mainSize > threshold.maxMain) {
    warnings.push(
      `Main bundle exceeds threshold: ${formatBytes(info.mainSize)} > ${formatBytes(threshold.maxMain)} (+${formatBytes(info.mainSize - threshold.maxMain)})`
    );
    pass = false;
  }
  ...
  if (info.totalSize > threshold.maxTotal) {
    warnings.push(
      `Total bundle exceeds threshold: ${formatBytes(info.totalSize)} > ${formatBytes(threshold.maxTotal)} (+${formatBytes(info.totalSize - threshold.maxTotal)})`
    );
    pass = false;
  }
