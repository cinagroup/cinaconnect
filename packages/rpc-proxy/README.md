...
RPC Proxy 是 OnChainUX 的 RPC 请求路由层。它智能地将请求分发到多个区块链 RPC 提供者，使用 Redis 缓存常见响应，并通过去重机制减少上游调用。
...
```bash
docker build -t onchainux/rpc-proxy:latest .
docker run -p 8545:8545 \
  ...
  onchainux/rpc-proxy:latest
```

### Kubernetes

```bash
helm install rpc-proxy ./deploy/helm/onchainux \
