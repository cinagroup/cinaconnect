...
> Generated API reference for `@onchainux/analytics`.
...
```bash
npm install @onchainux/analytics
```
...
```typescript
import { AnalyticsTracker } from '@onchainux/analytics'
...
|-------|---------------|------------|
| `app_initialized` | OnChainUX is initialized | `version`, `chains` |
...
function onUserOptOut() {
  analytics.disable()
  localStorage.removeItem('onchainux_analytics_id')
}
