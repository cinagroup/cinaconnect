...
**关键差异**: RainbowKit **仅支持 EVM 链**（约 35 条，全部已构建测试）。CinaConnect **架构设计覆盖 16 条跨链协议**（EVM + Solana + Bitcoin + TON + TRON + Cosmos + Sui + Starknet + NEAR + Hedera + XRPL），7+ 个 adapter 已构建，链适配器在 core-sdk 中被注释掉，当前实际可用的只有 core-sdk 内置的 EVM 基础支持。
...
**关键差异**: RainbowKit 钱包数量多但**仅限 EVM**，且**全部经过实际连接测试**。CinaConnect 架构覆盖所有链协议 — 30+ wallet connectors **已构建，未进行连接测试**。Demo app 的 WalletModal 是 **纯 mock UI**，不连接真实钱包。
...
**关键差异**: RainbowKit 只是一个 **前端 UI 组件库**，依赖 wagmi + viem + Rainbow RPC。CinaConnect 是**完整基础设施栈架构** — RPC Proxy 和 Keys Server 确实已部署在 Cloudflare Workers 上且可用，Relay Server 已构建，其余基础设施服务已构建。
...
| **CLI 工具** | ✅ create-rainbowkit | ✅ cinaconnect CLI | ✅ 已构建 |
...
| 多链架构（非 EVM 需求） | CinaConnect（64/64 已构建，适配器需启用） |
| 需要社交登录/Passkey | CinaConnect（已构建，Demo 为 mock） |
| 需要自托管基础设施 | CinaConnect（RPC + Keys 已部署，其余已构建） |
| 企业级全栈方案 | CinaConnect（64/64 已构建，约 75-80% 可用） |
| 追求最小依赖/最快上手 | RainbowKit |
| 成本敏感（需要 $0 运行） | CinaConnect（Cloudflare 免费额度，64/64 已构建） |
| 从其他钱包 SDK 迁移 | CinaConnect（迁移工具已构建） |
